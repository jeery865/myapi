...
- 🚦 **中转不比上游更严**：模型能不能用由上游说。引擎当前列表里没有的模型只标「未列出 · 仍可试」，照常转发 —— 额度每天刷新、付费能解锁，网关不替你判死刑
...
> 唯一一处例外是**下载后校正**：引擎里那份 `PAUSED_MODELS`（手写名单，命中就在引擎本地直接拒掉、请求发不到上游）会按官方 `FREEBUFF_PAUSED_FREE_MODEL_IDS` **整体对齐** —— 官方恢复上架的自动放开，官方新撤下的自动拦住。上游那份既没有回收机制（`deepseek-v4-flash` 恢复后还在名单里待了一个月），也不会补进新撤下的模型。对齐是双向的、结果确定性且幂等，**拿不到官方名单时一个字都不动**（`null` 是"没证据别动"，`[]` 才是"官方确实一个都没撤"）。实现见 `src/vendor-patch.js`；触发方式就是下面三种更新里的任意一种。
> 跟随上游的更新有三种触发方式：① 应用启动后每 6 小时自己从官方常量刷一次模型分类**和官方「已撤下」名单**（`src/models.js` 的 `refreshCatalog`，不需要重新部署）—— 撤下/恢复都会自动跟上，不用改任何配置；② 仓库里的 GitHub Actions（`.github/workflows/update-vendor.yml`）每天跑一次 `npm run update-worker` 并把结果提交回仓库，Railway 跟着重新部署；③ 本地 `npm run update-worker`。
> 官方「已撤下」在运行时是**真拦**、不只是标记：撤下的模型上游不报错，而是**静默降级到默认模型**，直接放过去会让客户端以为在用 A、其实拿回来的是 B。所以网关按官方名单在本地明确回一个 `unsupported_model`（状态码和错误 type 与引擎自己的暂停行为一致，客户端无感），控制台里标成 `withdrawn`、`/v1/models` 里不列出、也不会被选作降级目标。名单随上面那条每 6 小时自动刷新，官方恢复上架后同上自动放行。官方名单解析不出来时一律不拦（fail-open）—— 绝不把"不知道"当成"已撤下"。
...
- **不勾（默认）**：`/v1/models` 只返回免费模型，请求付费模型直接 403，不会消耗号池的 Premium 额度（`deepseek-v4-flash` 曾在 2026-08-18 被挪进 premium 池，2026-09 已回归免费）
...
|---|---|---|
| `premium` | **付费** | 全账号共享 **5 次 session/天**（2026 年一度是 4），太平洋日切换 |
| `deepseek/*` | 按模型块自己的 `premium` 标记 | 现在 Flash 已回归免费，只有 Pro 仍走 premium 池 |
| `standard` | 免费 | 非 premium 那一档：`deepseek/deepseek-v4-flash`、`mimo/mimo-v2.5`、`stealth/ox-alpha` |
| 限量试用 | 免费但会消失 | `claude-fable-5` 这类，上游全局池放多少算多少，池子空了模型就整个消失 |
| reward（旧名 `glm`） | 免费（需资格） | 官方 referral / streak 奖励的那一个：现在是 `z-ai/glm-5.3-flash`（取代了 GLM 5.2） |
| 表里没有的新模型 | 默认按**付费** | 保守策略，确认是免费的话在「模型」卡里改一下 |

**2026-09 上游做了一次重构，这个项目跟着修了一处解析 bug**：上游把「哪些模型算 premium」
从字面量数组改成了从 catalog 派生的表达式（`FREEBUFF_MODELS.filter(m => m.premium).map(m => m.id)`），
旧解析器读不到它，于是 premium 集合为空、整张表被静默判成「全部免费」——
没勾「允许付费」的 key 就会真的去烧 premium 额度。现在改成直接读每个模型块自己的
`premium` 字段（上游那句 `filter` 用的就是同一份元数据）。顺带一提，第三方那份
`freebuff-models.json` 的生成器也栽在同一个坑里（`pools.premium` 是空的），所以
`npm run update-worker` 会用官方常量把它的额度池补回来，避免「只收紧」底表被清空。

**2026-08-18 的变动**：Flash 一直是"不占额度的那个"，那天被挪进 premium 池，
官方注释写明是 **TEMPORARY**，原因是它成了免费模式最大的成本项。同一段注释还写着
*"Unlimited is MiMo 2.5 while this holds"* —— 所以那段时间真正不限量的是 MiMo 2.5。
本项目「客户端不写 model 时用哪个」默认就自动挑当前不限量的那一档（而不是死写 Flash），
免得每个不带 model 的请求都去啃那每天 1 次的 DeepSeek 额度。

**2026-09 又改回来了**：Flash 已经离开 premium 名单、回归免费，GLM 5.3 Flash 接替了
GLM 5.2 的位置（还顺带成了 referral 奖励的兑换目标）。这也正是为什么默认 model 不能写死 ——
上游一个月里把 Flash 挪进挪出，只有"跟着当前分类自动挑"才跟得上。

**官方撤下名单是自动跟的**：`FREEBUFF_PAUSED_FREE_MODEL_IDS` 是唯一权威的"这个模型现在还
算不算在售"判据 —— 别拿"在不在 standard / premium 池里"当判据：官方撤下模型时会**刻意保留**
它的 id 和 agent 映射（这样老客户端发来的请求能被降级到默认模型，而不是收到一个它不认识的
id 然后无限重试），所以撤下的模型照样出现在池子里。只有这份名单会说"它已经被端走了"。

它由 `refreshCatalog` 带着官方常量源一起拉，**每 6 小时自动刷一次** —— 官方今天撤下、明天
恢复，网关都会自动跟上，不需要重新部署、也不需要手动跑任何脚本：

| 环节 | 行为 |
| --- | --- |
| 控制台「模型」 | 状态列标成 `官方已撤下`，说明里写明判据和"会静默降级"这件事 |
| `GET /v1/models` | 不列出（那张表是"现在真能用的列表"） |
| 请求它 | 本地直接回 400 `unsupported_model`，**不转发给上游** |
| 模型降级 | 不会被选作降级目标 |
| 官方恢复上架 | 下一次自动刷新（≤6 小时）后自动变回可用，无需改配置 |

为什么要**真拦**、而不是像"引擎列表里没有它"（`未列出 · 仍可试`）那样只做提示：撤下的模型
上游**不报错**，而是**静默降级到默认模型** —— 客户端以为在用 A，其实拿回来的是 B 的回答，
那种沉默比一句明确的错误危险得多。但只有在官方名单**确实解析出来**时才拦；解析不出来
（网络故障、上游换了写法）就一律放行（fail-open），绝不把"不知道"当成"已撤下"。

`npm run update-worker` 会把同一份名单双向对齐进引擎的 `PAUSED_MODELS`（恢复的放开、新撤下的
补上），让"引擎本地拒"和"网关本地拒"用的是同一个判据；运行时不依赖它，所以漏跑一次也不影响。

**列表实不实用**：光有上游模型表不代表你的号现在真能用它。控制台的「实测状态」列汇总这些信号：

- 真实请求成功 → `实测可用`
- 0 消耗探活拿到的额度快照里出现过 → `上游有额度记录`
- 当前引擎的 `/v1/models` 里没列出它 → `未列出 · 仍可试`
- 上游按模型本身拒绝（`unsupported_model`、`session_model_mismatch` 之类）连续两次 → `实测不可用`

### 一条原则：中转不比上游更严

**「没列出」不等于「不能用」，所以这个网关不会因此拦你的请求。** 免费额度每天会刷新，
付费或升级账号也能解锁更多模型 —— 引擎的 `/v1/models` 是"此刻这个号能看到什么"的快照，
不是"这个模型存在与否"的判决。早期版本会把这类模型直接回 404 并标成「上游已暂停」，
那是错的：上游都没拦，中转凭什么拦。现在照常转发，能不能用由上游自己回答，
它的错误信息比我们猜的准。

唯一还会被自动藏起来的是**连续两次因模型本身被拒**（`实测不可用`）的，
而且这个也能在「设置」里关掉。真正的硬事实才会拦：
**官方已撤下**（400，判据是官方那份名单 —— 见上面「官方撤下名单是自动跟的」，
它同时会被标成 `withdrawn` 并从 `/v1/models` 藏掉）、
你在控制台手动下架的模型（403）、你自己停用的上游（503）、
opencode 那个按地区返回 403 的模型 —— 这些都是确定的，不是猜的。
...
- **四种原生协议全都承接了**：`claude-*` / `qwen*` 是 Anthropic 原生，`gpt-*` / `grok-*` / `muse-*` 是 Responses 原生，`gemini-*` 是 Google `generateContent` 原生，其余走 chat。这四套 body 格式都有适配器（`src/protocols/`），所以 Zen 上 63 个模型现在都能用 —— 早期只实现了 chat 和 anthropic，那时候会把 gpt / gemini 那批直接拒掉。Gemini 的端点还得按模型拼路径（`/models/<model>:generateContent`，流式换成 `:streamGenerateContent?alt=sse`），鉴权头也换成 `x-goog-api-key`。
...
## 三点八、出口代理（伪装 IP）：把出站流量走 Clash 订阅

控制台「设置 → 出口代理」。填一个 Clash 格式订阅地址 → 「保存并拉取」→ 打开「伪装 IP」→
在节点列表里点一个节点，之后**所有出站请求**（打上游的对话请求、探活、内置浏览器之外的网络调用）
都从那个节点的 IP 出去。

实现上是容器里多跑一个 [mihomo](https://github.com/MetaCubeX/mihomo) 内核：它把订阅里的
vmess / vless / trojan / ss 节点转成本地一个 HTTP/SOCKS 混合端口，网关的出站请求走这个端口。
只用混合端口模式，**不需要特权**（不装 TUN、不要 `NET_ADMIN`）。

### 先看清楚它能做到什么、做不到什么

这一节比上面那段重要，别跳过。

**它只换出口地址，不改变"同一个出口 + 多个账号"这个模式。** 上游判断"这是不是一个号池"，
看的是这个模式本身；把出口从 Railway 换成机场节点，模式一点没变。所以：

- 如果你的号池是「十几个号共用一个出口」，换 IP **基本不会有实质差别**。
- 机场节点几乎都是**数据中心 IP**，和 Railway 的出口同属性，风控对机房 IP 的多账号判定
  本来就比家宽更严。
- 机场节点是**很多人共享**的，里面难免有滥用者。如果上游有 IP 信誉库，这些 IP 可能比
  Railway 的出口**更脏**。

真正有实质意义的形态是**每个账号一个不同出口**（把「一 IP 多账号」拆成「一 IP 一账号」），
那需要按账号选 dispatcher —— 本功能是**单出口**，没做这件事。所以把它理解成
「出口 IP 池化 + 手动可切」，而不是「绕过风控」。

### 代价（都是真实的）

- 多一个常驻进程和一个**链路失败点**：节点掉线时，默认会让请求直接失败（见下）。
- 订阅方能看到你的**目标域名和请求时序**（TLS 隧道里看不到 token 内容，但知道你在打
  `api.cline.bot` 这类域名）。
- **跑代理转发可能违反 Railway 或节点提供商的服务条款**，自行判断。
- 镜像大 ~60MB，容器内存多一份内核。

### 代理不可用时怎么办

默认（「代理不可用时拒绝请求」开着）：**宁可整个请求失败，也不让流量从真实出口走**。
`/v1` 会返回 `503` + `{"type":"proxy_unavailable"}`，并且**不会写任何账号状态** —— 出口坏了
不该由某个号来背锅，否则整池会被连环标坏，而冷却到期后的探活还要走代理、继续失败，永远好不了。

关掉这个开关就回退直连：服务不会中断，但那段时间**暴露真实 IP**（也就是没开这个功能的状态）。

### 其他行为

- **不探活节点**。节点存活只由内核自己的健康检查（订阅里节点的 `alive`）反映，
  网关不会额外打流量去测 —— 测延迟要你手点。
- 订阅原文存在 `<DATA_DIR>/clash/sub.yaml`（**里面是节点密码，别把数据目录暴露出去**）。
  订阅 URL 本身常带机场凭据，所以控制台只显示域名（`/state` 不下发完整 URL）。
- mihomo 的控制 API 只监听 `127.0.0.1`，密码随机生成存 `<DATA_DIR>/clash/secret`；
  `allow-lan` 强制关（否则容器就成了一个开放代理）。
- 订阅必须是 **Clash 格式**（含 `proxies:` 段）。给的是 base64 或别的格式时，去机场后台
  换成「Clash」订阅链接。
- **控制台的「自检」和「检测」也走这道闸**。自检是真实请求（还会把整池凭据交给引擎），
  出口不可用时会被 `503` 拒绝并说明原因；「检测」同理，而且**不落账号状态** ——
  出口的锅不该记到号上。内核没跑时「刷新节点 / 切节点 / 测延迟」会直接给一句
  「内核没在运行，去点重新加载」，而不是抛一个裸的 `fetch failed`。
- **换订阅会真的重载内核**：点「保存并拉取」之后会显式让内核重读 `sub.yaml`，不用重启容器。
  （mihomo 只在启动时读一次文件，不补这一步的话节点列表和实际出口都还是旧的。）
- **内核残留占住端口时会明确报错**。上一次运行的 mihomo 没退干净（比如容器被 SIGKILL），
  新内核会绑不上端口；这时控制台直接说「端口已被占用」，而不是含糊的「启动超时」——
  后者会让你以为再等等就好，实际它一直在用那个旧实例。
- 订阅地址的**每一跳重定向**都会重新校验是不是公网地址（只认 https）—— 防止上游用 302
  把请求引到容器内网或云元数据端点。
...
| `MIHOMO_BIN` | `/usr/local/bin/mihomo` | 出口代理用的 mihomo 内核路径。镜像里已经装好；本机开发想试就指到本地那份 |
| `CLASH_MIXED_PORT` | `7890` | mihomo 的 HTTP/SOCKS 混合端口（只绑回环）。被占用就换一个 |
| `CLASH_API_PORT` | `9090` | mihomo 的控制 API 端口（只绑回环，带随机 secret） |
...
npm test                # 506 项：单测 196 + 协议适配器 154 + 协议端到端 69 + 集成 87
                        # 会自己起一个假上游和一个临时服务，不碰真网络
```

> 测试起的是一整个真服务（带控制台），所以它**只绑回环**（`HOST=127.0.0.1`）。
> 项目默认绑 `::`（全网卡，Railway 需要），在公网 IP 或开了端口映射的机器上
> 直接跑测试等于把控制台临时挂出去 —— 没必要，测试只会从 localhost 访问它。
...
**返回 429 / 额度用完？**
Premium 池目前是全账号共享 **5 次 session/天**（2026 年一度是 4，跟着上游常量动态变），按太平洋日切换（UTC+8 的 15:00 前后重置）。多加几个号，或者把 key 的「付费模型」取消勾选、只用免费模型。

**模型列表里有些模型根本调不通？**
上游模型表只说"官方有这个模型"，不代表你的号现在能用。但**网关不会替你判死刑**：当前引擎列表里没有的模型只标成「未列出 · 仍可试」，照样列给客户端、照样转发 —— 额度每天刷新、付费能解锁，能不能用由上游说。只有真实调用失败两次、且是模型本身被拒（不是额度或 token 问题）才标「实测不可用」并默认藏起来，这个开关在「设置」里。想把实测状态手动重来一遍就点「模型 → 清空实测状态」。

**为什么 Flash 一会儿免费一会儿付费？**
上游 2026-08-18 把它挪进 premium 池（注释写明是临时措施，因为它成了免费模式最大的成本项），
2026-09 又移出来回归免费 —— 一个月里挪进挪出。这不是本项目分类错了，就是上游在改；
所以「客户端不写 model 时用哪个」是**动态挑**当前不限量的那一档，而不是写死某个 id。
控制台「模型 → 列表怎么来的」里能看到当前表的来源、生成时间、引擎版本和真实额度数字。
